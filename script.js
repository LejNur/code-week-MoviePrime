...
const render = async (endpoint, query) => {
  ...
  results = response.results.map((movie) => ({
    ...movie,
    isFavorite: favoriteMovies.find((fav) => fav.id === movie.id),
  }));
...
const handleButtonClick = (event) => {
  if (event.target.textContent === "+") {
    ...
    dialogEl.textContent =
      translations[selectLanguageEl.value].addedToFavorites;
  ...
  if (event.target.textContent === "-") {
    dialogEl.showModal();
    dialogEl.textContent =
      translations[selectLanguageEl.value].confirmRemoveFromFavorites;
    ...
    buttonYes.addEventListener("click", () => {
      dialogEl.textContent =
        translations[selectLanguageEl.value].removedFromFavorites;
      movieObj = JSON.parse(event.target.dataset.item);
      results = results.map((movie) =>
...
const getGenreList = async () => {
  const genreList = await GET(`genre/${type}/list`, 1, "", language);
